...
(
    id        SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name      VARCHAR(100) UNIQUE NOT NULL,
    photo_url VARCHAR(1000) UNIQUE NOT NULL,
...
(
    ...
    phone         VARCHAR(16) NOT NULL
        CHECK (REGEXP_LIKE(phone, '^\\+7-\\d{3}-\\d{3}-\\d{2}-\\d{2}$')),
...
(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    club_id INT NOT NULL REFERENCES info.club(id),
    duration_minutes SMALLINT NOT NULL CHECK (duration_minutes >= 15),
...
(
    ...
    CHECK (
        (need_password = TRUE AND password IS NULL)
            OR
        (need_password = FALSE AND password IS NOT NULL)
        )
);
...
(
    ...
    employee_role VARCHAR(20) NOT NULL
        CHECK (employee_role IN ('EMPLOYEE','ADMIN','MANAGER')),
...
(
    ...
    pay_confirmed_by_employee_id BIGINT REFERENCES hr.employee(id),
